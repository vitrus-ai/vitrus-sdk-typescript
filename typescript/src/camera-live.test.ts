...
test('Droid.connect forwards camera.observeFrames only to the public dataplane',async()=>{
 const originalFetch=globalThis.fetch;const requests:Array<{url:URL;headers:Headers}>=[];
 globalThis.fetch=(async(input,init)=>{
  const url=new URL(String(input));requests.push({url,headers:new Headers(init?.headers)});
  if(url.pathname==='/v1/droids/resolve')return Response.json({id:'droid-1',serialNumber:'VTRS-R06',model:'r06',displayName:null,organizationId:'org-1',status:'online',enrollmentState:'enrolled'});
  if(url.pathname==='/v1/droids/cameras/live')return new Response(stream(event('public-frame','2026-09-10T00:00:05Z')));
  return Response.json({error:`unexpected ${url.pathname}`},{status:500});
 }) as typeof globalThis.fetch;
 try{
  const {Droid}=await import('./droid-live.js');
  const droid=await Droid.connect('VTRS-R06',{apiKey:'public-key'});
  const iterator=droid.camera.observeFrames('head_camera',{reconnect:false});
  const first=await iterator.next();await iterator.return?.();
  expect(first.value?.frameId).toBe('public-frame');
  const live=requests.find(request=>request.url.pathname==='/v1/droids/cameras/live')!;
  expect(live.url.origin).toBe('https://vitrus-dataplane.onrender.com');
  expect(live.url.searchParams.get('ref')).toBe('VTRS-R06');
  expect(live.headers.get('authorization')).toBe('Bearer public-key');
